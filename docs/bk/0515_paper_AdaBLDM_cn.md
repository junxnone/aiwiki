...
\\times 3} | i = 1,2, \\ldots, N\_{NG}^{\*}}$ 且
$M\_{NG}^{\*}={M\_{NG}^{\*i} \\in \\mathbb{B}^{H\_x \\times W\_x} | i =
1,2, \\ldots, N\_{NG}^{\*}}$ 分别包含所获得的合成样本和相应的缺陷掩码。此外，通常我们设置
$N\_{NG}^{\*} \\gg N\_{NG}$ 以确保生成的样本具有高度的模式变化。
