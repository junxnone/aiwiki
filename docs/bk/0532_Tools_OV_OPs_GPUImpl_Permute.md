...
| 格式兼容性  | 适配 GPU 所有存储格式：- 线性格式（bfyx/nhwc）；- 阻塞格式（b\_fs\_yx\_fsv16/b\_fs\_zyx\_fsv32）；无需先转回线性格式，直接在阻塞格式上完成维度置换。                        |
...
| reorder | 维度顺序 + 内存格式 + 数据类型转换 | 格式适配（如 bfyx→b\_fs\_yx\_fsv16）+ 类型转换（f32→f16） |
