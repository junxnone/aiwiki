...
Update Date | `2022-04-07T14:49:33Z`
...
- $h_{i,j} = p^1_{i,j} + p^2_{i,j}\theta_{i,j} + p^3_{i,j}\theta_{i,j}^2 + p^4_{i,j}\theta_{i,j}^3 $
- $h_{i,j}$ 为整个物体的高度矩阵
- $\theta_{i,j}$ 为计算的相位矩阵
- $p^1_{i,j} p^2_{i,j} P^3_{i,j} p^4_{i,j}$ 为相机参数/Pixel
